...
fn main() -> Result<()> {
    ...
    fn part1<R: BufRead>(reader: R) -> Result<usize> {
        ...
        //go through each range provided by the input
        for item in items {
            let range = item.split("-").collect::<Vec<&str>>();
            let start = range[0].parse::<usize>().unwrap();
            let end = range[1].parse::<usize>().unwrap();
            //go through each range from start to end to detect any which repeat twice
            for i in start..=end {
    ...
    fn part2<R: BufRead>(reader: R) -> Result<usize> {
        ...
        for item in items {
            ...
            let start = range[0].parse::<usize>().unwrap();
            let end = range[1].parse::<usize>().unwrap();
            //go through each range from start to end to detect repeating patterns
            for i in start..=end {
                let string = i.to_string();
                let string = string.as_str();
